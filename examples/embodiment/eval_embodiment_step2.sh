...
# LOG_DIR="${BASE_LOG_DIR}/libero_spatial_base_130_step2/step_0/"
# MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
# mkdir -p "${LOG_DIR}"
# CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} actor.model.num_steps=${NUM_STEPS}"
# echo ${CMD}
# ${CMD} 2>&1 | tee ${MEGA_LOG_FILE}

# CONFIG_NAME="libero_object_grpo_openpi_pi05"
# LOG_DIR="${BASE_LOG_DIR}/libero_spatial_base_130_step2/step_0_test_object_cl"
# MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
# mkdir -p "${LOG_DIR}"
# CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} actor.model.num_steps=${NUM_STEPS}"
# echo ${CMD}
...
# LOG_DIR="${BASE_LOG_DIR}/libero_spatial_base_130_step2/step_0_test_goal_cl"
# MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
# mkdir -p "${LOG_DIR}"
# CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} actor.model.num_steps=${NUM_STEPS}"
# echo ${CMD}
...
# LOG_DIR="${BASE_LOG_DIR}/libero_spatial_base_130_step2/step_0_test_long_cl"
# MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
# mkdir -p "${LOG_DIR}"
# CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} actor.model.num_steps=${NUM_STEPS}"
# echo ${CMD}
...
BASE_CHECKPONIT_PATH="${REPO_PATH}/logs/20260420-00:20:33-libero_spatial_offline_grpo_openpi_pi05_wo_vlm_step2/libero_spatial_offline_grpo_openpi_pi05/checkpoints"
...
for STEP in ${STEPS[@]}; do
    ...
    CONFIG_NAME="libero_spatial_grpo_openpi_pi05"
    LOG_DIR="${BASE_LOG_DIR}/libero_spatial_base_130_posttrain_spatial_step2/step_${STEP}" #/$(date +'%Y%m%d-%H:%M:%S')"
    MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
