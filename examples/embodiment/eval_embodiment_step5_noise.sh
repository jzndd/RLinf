...
BASE_DIR="${REPO_PATH}/logs/20260420-17:59:35-libero_spatial_offline_grpo_openpi_pi05_noise_wo_vlm_step5"
BASE_CHECKPONIT_PATH="${BASE_DIR}/libero_spatial_offline_grpo_openpi_pi05_flow_noise/checkpoints"
...
for STEP in ${STEPS[@]}; do
    ...
    mkdir -p "${LOG_DIR}"
    CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} runner.ckpt_path=${CHECKPONIT_PATH} actor.model.num_steps=${NUM_STEPS} actor.model.num_action_chunks=${NUM_ACTION_CHUNK} env.eval.total_num_envs=${EVAL_TIMES}"
    echo ${CMD}
    ${CMD} 2>&1 | tee ${MEGA_LOG_FILE}

    # EVAL 500 STEP performance
    CONFIG_NAME="libero_goal_grpo_openpi_pi05_noise"
    LOG_DIR="${BASE_LOG_DIR}/step_${STEP}_test_goal_cl" #/$(date +'%Y%m%d-%H:%M:%S')"
    MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
    CHECKPONIT_PATH="${BASE_CHECKPONIT_PATH}/global_step_${STEP}/actor/model_state_dict/full_weights.pt"
    mkdir -p "${LOG_DIR}"
    CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} runner.ckpt_path=${CHECKPONIT_PATH} actor.model.num_steps=${NUM_STEPS} actor.model.num_action_chunks=${NUM_ACTION_CHUNK} env.eval.total_num_envs=${EVAL_TIMES}"
    ...
    LOG_DIR="${BASE_LOG_DIR}/step_${STEP}_test_object_cl" #/$(date +'%Y%m%d-%H:%M:%S')"
    MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
    CHECKPONIT_PATH="${BASE_CHECKPONIT_PATH}/global_step_${STEP}/actor/model_state_dict/full_weights.pt"
    mkdir -p "${LOG_DIR}"
    CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} runner.ckpt_path=${CHECKPONIT_PATH} actor.model.num_steps=${NUM_STEPS} actor.model.num_action_chunks=${NUM_ACTION_CHUNK} env.eval.total_num_envs=${EVAL_TIMES}"
    ...
    LOG_DIR="${BASE_LOG_DIR}/step_${STEP}_test_long_cl" #/$(date +'%Y%m%d-%H:%M:%S')"
    MEGA_LOG_FILE="${LOG_DIR}/eval_embodiment.log"
    CHECKPONIT_PATH="${BASE_CHECKPONIT_PATH}/global_step_${STEP}/actor/model_state_dict/full_weights.pt"
    mkdir -p "${LOG_DIR}"
    CMD="python ${SRC_FILE} --config-path ${EMBODIED_PATH}/config/ --config-name ${CONFIG_NAME} runner.logger.log_path=${LOG_DIR} runner.ckpt_path=${CHECKPONIT_PATH} actor.model.num_steps=${NUM_STEPS} actor.model.num_action_chunks=${NUM_ACTION_CHUNK} env.eval.total_num_envs=${EVAL_TIMES}"
