...
WORLD_SIZE="${4:-8}"
BASE_LOG_DIR="${5:-${REPO_PATH}/logs/eval_libero_spatial_subset_model_path_base_pi05}"
DENOISE_STEP=${6:-2}

# Canonical user-facing task order for libero_spatial tasks.
ORDERED_TASK_DESCS=(
...
if [[ -z "${SUBSET_NAME}" || -z "${MODEL_PATH}" ]]; then
    ...
    echo "Example:"
    echo "  $0 subset1 /path/to/openpi/model libero_spatial_offline_grpo_openpi_pi05_noise_subset 8"
    exit 1
fi
