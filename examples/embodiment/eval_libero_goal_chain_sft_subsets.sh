...
EVAL_SCRIPT="${EMBODIED_PATH}/eval_libero_goal_subset_tasks_use_model_path.sh"

CONFIG_NAME="${1:-libero_goal_grpo_openpi_pi05_noise}"
WORLD_SIZE="${2:-8}"
BASE_LOG_DIR="${3:-${REPO_PATH}/logs/eval_libero_goal_chain_sft}"
DENOISE_STEP="${4:-2}"

OPENPI_ROOT="${OPENPI_ROOT:-/mnt/project_rlinf/jzn/workspace/openpi}"
START_SUBSET="${START_SUBSET:-1}"
END_SUBSET="${END_SUBSET:-1}"
DRY_RUN="${DRY_RUN:-0}"
ALLOW_MISSING="${ALLOW_MISSING:-0}"

CONFIG_PREFIX="pi05_libero_goal_subset"
EXP_PREFIX="libero_goal_chain_subset_new10_old5_"
...
RUN_GROUP_TAG="$(date +'%Y%m%d-%H%M%S')_libero_goal_chain_sft"

echo "============================================================"
echo "LIBERO goal chain SFT subset eval"
