...
from rlinf.workers.actor.fsdp_offline_fastwam_grpo_worker import (
    OfflineFastWAMGRPOActor,
)
...
def materialize_offline_dimensions(cfg):
    ...
    with open_dict(cfg):
        cfg.data.action_chunk = action_chunk
...
@hydra.main(
    version_base="1.1",
    config_path="config",
    config_name="libero_spatial_offline_grpo_fastwam_noise",
)
def main(cfg) -> None:
    ...
    actor_group = OfflineFastWAMGRPOActor.create_group(cfg).launch(
        cluster,
