...
CONFIG_NAME="${3:-libero_10_grpo_openpi_pi05_noise}"
WORLD_SIZE="${4:-8}"
BASE_LOG_DIR="${5:-${REPO_PATH}/logs/eval_libero_10_subset_model_path_base_pi05}"
DENOISE_STEP=${6:-2}

# Canonical user-facing task order for libero_10 tasks.
ORDERED_TASK_DESCS=(
...
if [[ -z "${SUBSET_NAME}" || -z "${MODEL_PATH}" ]]; then
    ...
    echo "Example:"
    echo "  $0 subset1 /path/to/openpi/model libero_10_offline_grpo_openpi_pi05_noise_subset 8"
    exit 1
fi
...
case "${SUBSET_NAME}" in
    subset1|libero_10_fullshot_subset1) MAX_TASK_ID=5 ;;
    subset2|libero_10_fullshot_subset2) MAX_TASK_ID=6 ;;
    subset3|libero_10_fullshot_subset3) MAX_TASK_ID=7 ;;
    subset4|libero_10_fullshot_subset4) MAX_TASK_ID=8 ;;
    subset5|libero_10_fullshot_subset5) MAX_TASK_ID=9 ;;
    *)
...
for user_task_id in range(max_user_task_id + 1):
    ...
    if task_desc not in desc_to_task_id:
        raise KeyError(f"Task description not found in libero_10 benchmark: {task_desc}")
...
echo
echo "User task to benchmark task mapping (from libero_10 benchmark):"
